...
import { FILIERES, calculateScore, emptyInputs, formatNumber, type FiliereCode, type ScoreInputs, type ScoreResult } from '@/data/calculator';
...
export function CalculatorPage() {
  ...
  const changeFiliere = (filiere: FiliereCode) => {
    setInputs((current) => ({
      ...current,
      filiere,
      bac: '',
  ...
  const validateStep = () => {
    ...
    if (step === 0) {
      ...
      else if (!FILIERES[inputs.filiere].eligibleBaccalaureates.includes(inputs.bac)) {
        nextErrors.push('Sélectionnez un baccalauréat admissible pour cette filière.');
      }
    }
    if (step === 1) {
    ...
    <div className="mx-auto max-w-6xl px-5 py-10 lg:px-8 lg:py-16">
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[1fr_290px]">
        <section className="min-h-[430px] rounded-[24px] border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-5 shadow-[0_14px_35px_hsl(var(--foreground)/.045)] sm:p-8">
          {step === 0 && <div className="fade-up"><div className="mb-7"><h2 className="font-display text-2xl font-extrabold">Commençons par votre profil</h2><p className="mt-2 text-sm leading-6 text-[hsl(var(--muted-foreground))]">Ces informations déterminent les matières et coefficients affichés ensuite.</p></div><FieldLabel>Votre filière</FieldLabel><div className="grid gap-3 sm:grid-cols-2">{(Object.keys(FILIERES) as FiliereCode[]).map((code) => <button type="button" key={code} onClick={() => changeFiliere(code)} data-testid={`button-filiere-${code}`} className={`rounded-2xl border p-4 text-left transition-all ${inputs.filiere === code ? 'border-[hsl(var(--primary))] bg-[hsl(var(--secondary)/.68)] ring-2 ring-[hsl(var(--primary)/.15)]' : 'border-[hsl(var(--border))] hover:border-[hsl(var(--primary)/.5)]'}`}><div className="flex items-center justify-between"><span className="font-mono text-lg font-bold text-[hsl(var(--primary))]">{code}</span>{inputs.filiere === code && <CheckCircle2 size={18} className="text-[hsl(var(--primary))]" />}</div><div className="mt-2 text-sm font-bold">{FILIERES[code].label}</div><div className="mt-1 text-xs leading-5 text-[hsl(var(--muted-foreground))]">{FILIERES[code].description}</div></button>)}</div><div className="mt-7 rounded-2xl bg-[hsl(var(--secondary)/.5)] p-4"><div className="text-xs font-bold uppercase tracking-[.12em] text-[hsl(var(--primary))]">Baccalauréats admissibles</div><ul className="mt-3 grid gap-2 text-sm text-[hsl(var(--muted-foreground))] sm:grid-cols-2">{FILIERES[inputs.filiere].eligibleBaccalaureates.map((bac) => <li key={bac} className="flex items-start gap-2"><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-[hsl(var(--accent))]" />{bac}</li>)}</ul></div><div className="mt-7"><FieldLabel hint="selon votre relevé">Type de baccalauréat</FieldLabel><select aria-label="Type de baccalauréat" data-testid="select-baccalaureat" value={inputs.bac} onChange={(event) => update('bac', event.target.value)} className="w-full rounded-xl border border-[hsl(var(--input))] bg-[hsl(var(--card))] px-4 py-3 text-sm outline-none focus:border-[hsl(var(--primary))]"><option value="">Sélectionnez votre baccalauréat</option>{FILIERES[inputs.filiere].eligibleBaccalaureates.map((bac) => <option value={bac} key={bac}>{bac}</option>)}</select></div></div>}
          {step === 1 && <div className="fade-up"><div className="mb-7"><h2 className="font-display text-2xl font-extrabold">Votre parcours et vos notes</h2><p className="mt-2 text-sm leading-6 text-[hsl(var(--muted-foreground))]">Une seule situation de redoublement peut être sélectionnée.</p></div><FieldLabel>Redoublement</FieldLabel><div className="grid gap-2 sm:grid-cols-3">{[['none', 'Aucun', 'N1 = 10'], ['premiere', '1ère année du bac', 'N1 = 5'], ['terminale', 'Terminale', 'N1 = 0']].map(([value, title, detail]) => <button type="button" key={value} onClick={() => update('repetition', value as ScoreInputs['repetition'])} data-testid={`button-repetition-${value}`} className={`rounded-xl border px-4 py-4 text-left ${inputs.repetition === value ? 'border-[hsl(var(--primary))] bg-[hsl(var(--secondary)/.68)]' : 'border-[hsl(var(--border))]'}`}><div className="text-sm font-bold">{title}</div><div className="mt-1 font-mono text-xs text-[hsl(var(--primary))]">{detail}</div></button>)}</div><div className="mt-9 grid gap-4 sm:grid-cols-2"><div><FieldLabel hint="/ 20">M1 · Moyenne du passage du 1-BAC à la terminale</FieldLabel><NumberInput value={inputs.m1} onChange={(value) => update('m1', value)} max={20} label="Note M1" testId="input-m1" /></div><div><FieldLabel hint="/ 20">M2 · Moyenne générale du baccalauréat</FieldLabel><NumberInput value={inputs.m2} onChange={(value) => update('m2', value)} max={20} label="Note M2" testId="input-m2" /></div></div><div className="mt-8 rounded-2xl bg-[hsl(var(--secondary)/.55)] p-4 text-sm leading-6 text-[hsl(var(--muted-foreground))]"><span className="font-mono font-bold text-[hsl(var(--primary))]">N2</span> = (M1 + 2 × M2) / 3 · La deuxième année compte double.</div></div>}
